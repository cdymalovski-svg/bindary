...
export default function Editor() {
  ...
  // Auto-save: debounced 1.2s after the last edit. Hook skips the first
  // effect run (right after load) so a freshly-fetched book isn't
  // re-saved unnecessarily. The returned `skipNextAutoSaveRef` lets
  // callers defer autosave for programmatic setBook() calls (TOC
  // refresh, history restore) that aren't user edits.
  const { skipNextRef: skipNextAutoSaveRef } = useAutoSave(book, loading, saveBook);
  ...
  // --- PDF export ---
  // Encapsulated in a hook so the ~400-line polling + toast + print/preview/download
  // decision tree lives outside this file. Exposes `exporting` (to disable the toolbar
  // Export button), `exportsBump` (bumped after success so the popover history refetches),
  // and the file-health warning-dialog state (`fileHealthProblems` + cancel/proceed).
  const {
    onExportPdf,
    exporting,
    exportsBump,
    ...
    <div className="h-screen w-screen flex flex-col bg-desk overflow-hidden">
      ...
      <header className="h-14 border-b border-rule bg-paper flex items-center z-20 min-w-0">
        ...
        <FileHealthWarningDialog
          ...
          onCancel={onFileHealthCancel}
          onProceed={onFileHealthProceed}
        />
        </div>
