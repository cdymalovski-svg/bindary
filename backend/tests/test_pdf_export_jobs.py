...
see a request that runs longer than their idle/response timeout.
   ...
   POST /api/books/{id}/export.pdf/start       → { job_id, status: "pending" }
   GET  /api/books/{id}/export.pdf/status/{j}  → { status: "pending|ready|failed", … }
   GET  /api/books/{id}/export.pdf/download/{j}→ application/pdf
"""
import os
import time
...
def _wait_for_ready(session, book_id: str, job_id: str, timeout: float = 60.0) -> dict:
    ...
    while time.time() < deadline:
        r = session.get(f"{API}/books/{book_id}/export.pdf/status/{job_id}", timeout=15)
...
class TestPdfExportJobs:
    def test_start_returns_pending_job(self, s):
        r = s.post(f"{API}/books/{SMALL_BOOK_ID}/export.pdf/start", timeout=15)
        assert r.status_code == 200, r.text
        body = r.json()
    ...
    def test_start_404_for_unknown_book(self, s):
        r = s.post(f"{API}/books/does-not-exist/export.pdf/start", timeout=15)
        assert r.status_code == 404

    def test_status_404_for_unknown_job(self, s):
        r = s.get(f"{API}/books/{SMALL_BOOK_ID}/export.pdf/status/not-a-real-job", timeout=15)
        assert r.status_code == 404

    def test_full_flow_start_poll_download(self, s):
        # 1) start
        start = s.post(f"{API}/books/{SMALL_BOOK_ID}/export.pdf/start", timeout=15)
        ...
        # 3) download
        dl = s.get(f"{API}/books/{SMALL_BOOK_ID}/export.pdf/download/{job_id}", timeout=30)
        ...
        again = s.get(f"{API}/books/{SMALL_BOOK_ID}/export.pdf/download/{job_id}", timeout=10)
    ...
    def test_download_409_when_still_pending(self, s):
        ...
        job_id = start.json()["job_id"]
        try:
            dl = s.get(f"{API}/books/{SMALL_BOOK_ID}/export.pdf/download/{job_id}", timeout=10)
            ...
            s.get(f"{API}/books/{SMALL_BOOK_ID}/export.pdf/download/{job_id}", timeout=30)
